...
# bmi --------------------------------------------------------------------------
bmi <- open_dataset(file.path(raw_directory, "bmi")) |>
    mutate(person_id = as.character(person_id)) |>
    filter(
        person_id %in%
            cohort[, person_id] &
...
# ses --------------------------------------------------------------------------
ses <- open_dataset(file.path(raw_directory, "ses")) |>
    mutate(person_id = as.character(person_id)) |>
    filter(person_id %in% cohort[, person_id]) |>
    rename(
...
tmp <- icd[icd[, .I[which.min(date)], .(person_id)][, V1], ]
...
analytic_dataset[, `:=`(
    ...
    encounters_cat = factor(
        fcase(
            encounters < 50,
            "Less than 50",
            encounters >= 50 & encounters < 100,
            "[50, 100)",
            encounters >= 100 & encounters < 200,
            "[100, 200)",
            encounters >= 200,
            "200 and more"
        ),
...
var_label(analytic_dataset$encounters) <- "Number of encounters"
var_label(
    ...
    tbl_summary(
        ...
        (\(z) {
            if (!is.null(mod_span_head)) {
                z |> modify_spanning_header(mod_span_head)
            } else {
                z
            }
        })() |>
...
sema_oth_table <- analytic_dataset[
    ...
    tab_sum(
        by_var = "sema_vs_oth",
        mod_span_head = c("stat_1", "stat_2", "stat_2") ~ "**Drug type**"
    )


gt::gtsave(
    data = as_gt(summary_table),
    filename = "summary_table1.html",
    path = "tables/"
)

gt::gtsave(
    data = as_gt(demo_table),
    filename = "demo_table1.html",
    path = "tables/"
)

gt::gtsave(
    data = as_gt(comorbid_table),
    filename = "comorbid_table1.html",
    path = "tables/"
)

gt::gtsave(
    data = as_gt(lab_table),
    filename = "lab_table1.html",
    path = "tables/"
)

gt::gtsave(
    data = as_gt(sema_table),
    filename = "sema_table1.html",
    path = "tables/"
)

gt::gtsave(
    data = as_gt(sema_oth_table),
    filename = "sema_vs_oth_table1.html",
    path = "tables/"
)
