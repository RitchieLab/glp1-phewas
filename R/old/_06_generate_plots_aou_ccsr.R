...
ccsr_cols <- fread(
    "/Users/maxsalvatore/Library/CloudStorage/Box-Box/projects/GLP-1 RA/PheWAS/tables/ccsr_categories.csv"
) |>
    rename(group = short_description)
ccsr_labs <- fread(
    "/Users/maxsalvatore/Library/CloudStorage/Box-Box/projects/GLP-1 RA/PheWAS/tables/DXCCSR_v2025-1.csv"
) |>
    clean_names() |>
    remove_single_quotes() |>
    select(
        ccsr_code = ccsr_category_1,
        description = ccsr_category_1_description
    ) |>
    unique() |>
    mutate(abbrev = substr(ccsr_code, 1, 3))
ccsr_labels <- merge(
    ccsr_labs,
    ccsr_cols[, !c("long_description")],
    by = "abbrev"
) |>
    arrange(groupnum, ccsr_code) |>
    mutate(order = 1:n())

not_these <- c("END002", "END003", "SYM018", "END009")
...
quick_plot <- function(
    ...
    if (no_ob_osa) {
        dt <- dt[
            !(phecode %in% not_these),
        ]
    }

    itt_plot <- dt[type == "ITT" & ph_test_p > 0.05, ] |>
        plot_ccsr(ccsrx = ccsr_labels, phe_var = "phecode") +
        labs(
            title = paste0(
                "A. ",
                treatment,
...
sglt2 <- fread(file.path(
    project_directory,
    "results",
    paste0("glp_1_ra_sglt2i_ccsrwas_", version, ".csv")
))[, `:=`(
    log10p = log10(p),
...
dpp4 <- fread(file.path(
    project_directory,
    "results",
    paste0("glp_1_ra_dpp4i_ccsrwas_", version, ".csv")
))[, `:=`(
    log10p = log10(p),
    n_total = n_cases + n_controls
)][]
...
sema_sglt2 <- fread(file.path(
    project_directory,
    "results",
    paste0("semaglutide_sglt2i_ccsrwas_", version, ".csv")
))[, `:=`(
    log10p = log10(p),
    n_total = n_cases + n_controls
)][]
sema_dpp4 <- fread(file.path(
    project_directory,
    "results",
    paste0("semaglutide_dpp4i_ccsrwas_", version, ".csv")
))[, `:=`(
    log10p = log10(p),
    n_total = n_cases + n_controls
)][]
...
ggsave(
    file.path(
        project_directory,
        "figures",
        paste0("glp1_sglt2_ccsrwas_", version, "_plot.pdf")
    ),
...
ggsave(
    file.path(
        project_directory,
        "figures",
        paste0("glp1_dpp4i_ccsrwas_", version, "_plot.pdf")
    ),
    dpp4i_plots[["patched"]],
    width = 12,
    height = 6,
...
ggsave(
    file.path(
        project_directory,
        "figures",
        paste0("sema_sglt2_ccsrwas_", version, "_plot.pdf")
    ),
...
ggsave(
    file.path(
        project_directory,
        "figures",
        paste0("sema_dpp4i_ccsrwas_", version, "_plot.pdf")
    ),
...
ggsave(
    file.path(
        project_directory,
        "figures",
        paste0("glp1_ccsrwas_", version, "_plot.pdf")
    ),
    glp1_patched,
    width = 12,
    height = 8,
...
ggsave(
    file.path(
        project_directory,
        "figures",
        paste0("sema_ccsrwas_", version, "_plot.pdf")
    ),
    sema_patched,
    width = 12,
    height = 8,
